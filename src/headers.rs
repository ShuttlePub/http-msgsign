...
pub(crate) fn extract_content_digest(map: &http::HeaderMap) -> Result<ContentDigest, ExtractHeaderError> {
    let header = match map.get(CONTENT_DIGEST).map(|header| header.to_str()) {
        ...
        None => return Err(ExtractHeaderError::NoExist { header_name: CONTENT_DIGEST })
    };
    
    let dictionary = sfv::Parser::new(header.as_bytes())
        .parse_dictionary()?;
    ...
    let digest = match digest { 
        sfv::ListEntry::Item(sfv::Item { bare_item: sfv::BareItem::ByteSequence(bytes), .. }) => bytes,
